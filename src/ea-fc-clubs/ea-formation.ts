...
export function formationShape(positions: string[]) {
  ...
  for (const value of positions) {
    ...
    else if (/(CB|LB|RB|LWB|RWB|SW|DEFENDER|DEF)/.test(position)) counts.defense += 1;
...
export function selectBestFormation(matches: FormationCandidateMatch[]) {
  const candidates = new Map<
    string,
    { formation: string; matches: number; wins: number; draws: number; latestAt: number }
  >();
  ...
  const selected = [...candidates.values()].sort((a, b) => {
    return b.matches - a.matches || b.wins - a.wins || b.draws - a.draws || b.latestAt - a.latestAt;
  })[0];
